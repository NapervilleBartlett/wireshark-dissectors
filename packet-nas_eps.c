 ...
 *
 * $Id: packet-nas_eps.c 33455 2010-07-06 13:21:34Z etxrab $
 *
...
/* Utility functions */
/* MQ Orig  rededined the below 2 functioopms
static guint8
...
calc_bitrate(guint8 value){
        ...
        if ( (value > 0) && (value <= 63)) { valx = value ; }

        if (value > 63 && value <= 127) {
                valx  = 64 + (value - 64) * 8;
        }
        if (value > 127 && value <= 254) {
                valx  = 576 + (value - 128) * 64;
        }
    ...
    switch (octet&0x7){
		...
		case 6:
			...
			proto_tree_add_item(tree, hf_nas_eps_emm_m_tmsi, tvb, curr_offset, 4, FALSE);
                        ...
                        /* MQ */
                        if (check_col(gpinfo->cinfo, COL_INFO))
                                col_append_fstr(gpinfo->cinfo, COL_INFO, "M-TMSI=%2.2x%2.2x%2.2x%2.2x ",
                                        tvb_get_guint8(tvb, curr_offset),
                                        tvb_get_guint8(tvb, curr_offset+1),
                                        tvb_get_guint8(tvb, curr_offset+2),
                                        tvb_get_guint8(tvb, curr_offset+3));
	...
	switch(pdn_type){
		case 1:
			...
			proto_tree_add_item(tree, hf_nas_eps_esm_pdn_ipv4, tvb, curr_offset, 4, FALSE);
                        /* MQ */
                        if (check_col(gpinfo->cinfo, COL_INFO))
                                col_append_fstr(gpinfo->cinfo, COL_INFO, "UE IP=%d.%d.%d.%d ",
                                        tvb_get_guint8(tvb, curr_offset),
                                        tvb_get_guint8(tvb, curr_offset+1),
                                        tvb_get_guint8(tvb, curr_offset+2),
                                        tvb_get_guint8(tvb, curr_offset+3));
...
dissect_nas_eps_emm_msg(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, int offset, gboolean second_header)
{
	...
	if(second_header){
		...
		offset++;
                ...
                /* MQ Reset the security_header_type to 0 */
                security_header_type = 0 ;
                if (oct == 0xC7) {
                        /* Ser req is special. The MT is in header. So stop the offset++ */
                        offset--;
                }
...
dissect_nas_eps(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree)
{
	...
	if (security_header_type == 0){
		...
		if (security_header_type == 12){
                        /* MQ 4/22/10. Added 3 lines below */
                        if (check_col(pinfo->cinfo, COL_INFO)){
                                col_append_fstr(pinfo->cinfo, COL_INFO, " Service Request ");
                        }

                        proto_tree_add_text(tree, tvb, offset, 1, "NAS EPS Mobility Management Message Type: Service Request"); /* MQ */
